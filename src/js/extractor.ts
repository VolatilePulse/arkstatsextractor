...
// Extract creature base & domestic levels
export function ExtractLevelsFromTorpor(level: number, torpor: Interval, torporInc: Interval, imprint: Interval, m: Multipliers, canLevel: boolean): Array<[number, number]> {
    if (canLevel) throw new Error("Torpor being leveled is unsupported at this time.");
    if (m.Tm != IA.ZERO) throw new Error('Torpor having a TameMultiplier is unsupported at this time.');
